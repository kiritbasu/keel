---
name: keel-adopt
description: Use when putting an existing project into Keel for the first time — a repository with history, docs, ADRs and a backlog that predate Keel. Triggers on "adopt Keel", "set Keel up for this repo", "backfill Keel", "import our docs into Keel", "start using Keel on this project", or a Keel project that exists but is empty while the repository plainly is not. Not for the everyday loop; the `keel` skill covers that.
---

# Adopting Keel on a project that already exists
...
```bash
keel import docs/architecture.md docs/api.md --project <slug> --as spec --dry-run
```
...
would claim. That path is what `keel generate` later writes back over, so a
...
`keel generate <slug>` writes the mirror. Then look at the diff: it is the first
...
**Git history as events.** The event log is Keel's own audit trail of writes *to
Keel*. Filling it from commits does not record history, it fabricates an audit
...
thing Keel is for. You do not have that problem, and it is the reason the
