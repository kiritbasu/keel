...
> **Six rows in this table now argue from something that is gone**, and they are left exactly as they are pending KB's agreement, because the standing contract says spec decisions are his. Specifically: D-1a's closing clause says §3 onwards still describes the old shape, which stopped being true with this revision; D-2 and D-2b say "dataset" where they now mean a table; D-4's rationale rests on a DuckDB version conflict; D-5's rests on Quack; and D-6's "storage engines are Rust-native" is now a C amalgamation compiled into the binary. Each decision's *conclusion* survives the change of engine — that is the interesting part, and it is why nothing here has been quietly reworded to match. The sections above carry the corrected reasoning.
