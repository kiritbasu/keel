...
## What Keel is, in terms of what that risks

Keel is a local-first store. The daemon binds `127.0.0.1` and holds one SQLite
file under `~/.keel`. There is no account, no server, and nothing multi-tenant,
...
- **Anything that reaches the store from outside the machine.** The MCP endpoint
  ...
  without it is not.
- **Anything that reads or writes a file outside the paths Keel is supposed to
  touch.** Stored values name files Keel later writes, and they arrive from a
...
- **The update path.** Releases are verified against the SHA-256 in the release
  manifest. A way to make Keel install bytes that do not match, or to make it
...
The latest release. Keel is 0.x and there are no maintained branches behind it;
