//! `keel bootstrap` — seed Keel's own project. The dogfooding switch.
//!
//! The handoff calls this out: once Phase 1 exits, Keel tracks its own
//! development, and `product/STATUS.md` becomes something `keel render-status`
...
//! Keel is the source of truth and those files are outputs.
//!
...
pub fn run(store: &mut DuckStore, repo_path: Option<String>) -> Result<Summary> {
    ...
    let rows: Vec<Row<'_>> = vec![
        ...
        (
            0,
            "keel fsck — cross-engine referential integrity",
    ...
    let specs_src: [(&str, SpecKind, SpecStatus, &str); 3] = [
        ...
        (
            ...
            SpecKind::Note,
            SpecStatus::Draft,
            "Phase 2's criterion — nine of ten *unprompted* sessions write to Keel — cannot be \
    ...
    let terms: [(&str, &str); 12] = [
        ...
        (
            "Session",
            "One Claude conversation, used as the provenance unit. Caller-supplied; Keel never invents one.",
        ),
        (
            "Anchor",
        ...
        (
            "Phase gate",
            "The exit criterion for a build phase. Two of Keel's cannot be verified without a human.",
        ),
    ];
    ...
    let metrics: [(&str, &str, f64, MetricDirection, &[f64]); 4] = [
        (
            "Sessions where Claude writes to Keel unprompted",
            "%",
            80.0,
            MetricDirection::Up,
            &[],
...
pub fn archive_other_projects(store: &mut DuckStore, keep: &EntityId) -> Result<usize> {
...
mod tests {
    ...
    fn archiving_other_projects_leaves_the_named_one_alone() {
        ...
        // The fixture's demo project is also called `keel`, so the bootstrap
        ...
        let live = store
            ...
            .unwrap();
        assert_eq!(live.items.len(), 1, "only Keel should remain visible");
