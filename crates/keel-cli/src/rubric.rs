...
//! is indistinguishable in the log from one that never noticed Keel existed.
...
//! - **L1** — no sign of noticing Keel. The orientation mechanism failed.
//! - **L2** — read Keel, wrote nothing, never raised it. Noticed, judged not.
...
pub enum Level {
    ...
    L0NothingToRecord,
    /// No sign of noticing Keel at all.
    L1NoSignOfNoticing,
    /// Read Keel, wrote nothing, never raised it.
    L2NoticedNoDraft,
...
pub struct SessionRead {
    ...
    /// Keel tools the session actually invoked.
...
const WRITE_TOOLS: [&str; 4] = ["keel_create", "keel_update", "keel_write_doc", "keel_link"];
...
/// Whether a tool name writes.
pub fn is_write_tool(name: &str) -> bool {
    WRITE_TOOLS.iter().any(|w| name.ends_with(w))
}
...
mod known_answer_fixtures {
    ...
    fn offers_separate_the_permission_failure_from_the_orientation_failure() {
        ...
        for n in 5..10 {
            ...
            .unwrap();
            reads.push(read_transcript(&p).unwrap()); // never touched Keel
        }
        for r in &mut reads {
            r.level = classify(&r.keel_tools, r.write_attempts, false, &r.offers, true);
        }
