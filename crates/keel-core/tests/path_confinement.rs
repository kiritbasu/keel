...
//! at `~/.zshenv` and `keel generate` would put a document body there, to be
...
fn a_relative_root_path_is_refused_but_an_absolute_one_is_not() {
    ...
    let mut bad = Map::new();
    bad.insert("root_path".to_owned(), json!("development/keel"));
    assert!(
        store
    ...
    good.insert("root_path".to_owned(), json!("/Users/kb/development/keel"));
    store
...
/// validator existed, or by `keel import` — still cannot escape, because the
