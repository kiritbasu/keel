...
impl Fixture {
    ...
    fn set_noun(&mut self, project: &EntityId, noun: &str) -> keel_core::Result<Entity> {
    ...
    fn resolve(&self, word: &str) -> keel_core::Resolved {
...
fn a_projects_noun_resolves_even_with_no_term_for_it() {
    ...
    // project's own word is consulted before Keel's list, which matters for a
    ...
    assert_eq!(
        ...
        "a word Keel has never heard of still works, because the project said it"
    );
}
