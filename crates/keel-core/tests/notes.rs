...
use keel_core::{
    Actor, DuckStore, EntityStore, EntityType, NewNote, Project, Provenance, Surface, Task,
};

fn store() -> (tempfile::TempDir, DuckStore, keel_core::EntityId) {
...
fn a_note_on_a_nonexistent_row_is_refused() {
    ...
    let prov = Provenance::anonymous(Actor::Claude);
    let ghost = keel_core::EntityId::generate(EntityType::Task);
...
fn the_rendered_tracker_carries_the_notes() {
    ...
    let rendered = keel_core::render_status::render(&store, &project).unwrap();
