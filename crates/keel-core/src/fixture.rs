...
pub fn load<S: EntityStore + DocumentStore>(store: &mut S) -> Result<FixtureSummary> {
    ...
    // ---------------------------------------------------------------- Keel
    let mut keel = Project::new("keel", "Keel");
    keel.description = Some(
    ...
    keel.repo_urls = vec!["https://github.com/kb/keel".to_owned()];
    keel.aliases = vec!["project spine".to_owned(), "the keel store".to_owned()];
    keel.root_path = Some("~/development/keel".to_owned());
    let keel_id = make(store, keel.into(), &human, &mut s)?;
    ...
    let tasks: Vec<TaskRow<'_>> = vec![
        ...
        (
            ...
            "Phase 2 is the real test. If Keel is not useful with no UI, the premise is wrong.",
            TaskStatus::Todo,
            TaskPriority::P0,
            TaskKind::Task,
            &["plugin"],
        ...
        (
            ...
            "The diff already exists in keel-core; this is the rendering.",
            TaskStatus::Todo,
            TaskPriority::P2,
            TaskKind::Task,
            &["desktop"],
    ...
    let questions: Vec<QuestionRow<'_>> = vec![
        (
            ...
            QuestionStatus::Open,
            None,
            "Working assumption is ~/.keel with a local git repo and no remote. Moving it is a config change, so the cost of being wrong is low.",
        ),
        (
            &keel_id,
    ...
    let terms: Vec<TermRow<'_>> = vec![
        ...
        (
            None,
            "Soft delete",
            "Marking a row archived rather than removing it. Nothing in Keel is ever deleted.",
            &["archive"],
        ),
        (
            Some(&keel_id),
