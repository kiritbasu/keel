...
/// KEEL-116 established that a session told "you said 'sprint' — in Keel that is
...
/// glossary says so" is actionable in a way "because Keel accepts it" is not.
...
pub enum Source {
    ...
    ProjectNoun,
    /// Keel's built-in list of words projects commonly use.
    BuiltIn,
}
...
impl Source {
    ...
    pub const fn because(self) -> &'static str {
        match self {
            ...
            Source::BuiltIn => "Keel recognises that word",
        }
    }
}
...
pub fn resolve_type(
    ...
    // Keel's built-in list, and then the error it already raised.
