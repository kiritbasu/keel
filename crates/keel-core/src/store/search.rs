...
impl Store {
    ...
    ///
    /// The embedder is a parameter rather than state because `keel-core` is
...
mod tests {
    ...
    fn a_project_filter_and_a_type_filter_both_bite() {
        let (store, keel) = store_with_a_project();
        let other = add_project(&store, "widgets", "Widgets");
        let mine = add_task(&store, &keel, "Ship the widget", "a widget for the board");
