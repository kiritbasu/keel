...
/// requirement belongs where it can be met, and `keel lint` reports whatever
...
mod tests {
    ...
    fn an_update_that_changes_nothing_bumps_no_version_and_writes_no_event() {
        ...
        let mut changes = serde_json::Map::new();
        changes.insert("name".to_owned(), json!("Keel"));
    ...
    fn a_number_is_never_reused_after_a_row_is_archived() {
        ...
        assert_eq!(
            t.number, 2,
            "KEEL-1 must mean the same task forever, archived or not"
        );
    }
    ...
    fn a_readable_reference_resolves_and_nonsense_resolves_to_nothing() {
        ...
        assert_eq!(s.resolve_ref("KEEL-1").unwrap().as_ref(), Some(&task_id));
        assert_eq!(
        ...
        // reference.
        assert_eq!(s.resolve_ref("KEEL-999").unwrap(), None);
    ...
    fn a_second_project_with_the_same_letters_gets_a_distinct_key() {
        ...
        project(&mut s);
        let other = s
            .create(Project::new("keelboat", "Keelboat").into(), &claude())
            .unwrap();
