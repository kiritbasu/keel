...
//!
//! The SQLite twin of [`crate::store::rows`], and deliberately a twin rather
//! than a rewrite: the column lists themselves — [`TableSpec`], [`Col`] and
//! [`spec_for`] — are imported from there and not copied. Thirteen column
//! orders maintained in two places is thirteen chances for the two engines to
//! disagree about what a row is, and the disagreement would only show up as
//! data that survived the migration in the wrong fields.
//!
//! The two rules that keep the DuckDB file honest apply here unchanged:
//!
...
//!
//! **Lists.** DuckDB has a real `VARCHAR[]`, so the DuckDB file wraps list
//! columns in `to_json(…)` on the way out and rebuilds them with
//! `json_extract_string(?, '$[*]')` on the way in. SQLite has no array type, so
//! the column simply *is* JSON text. That makes the select list plain column
//! names and the insert plain placeholders — the conversion happens in Rust, at
//! [`arr`] and [`get_arr`], where it can be tested without a database.
//!
//! **Timestamps.** These are TEXT, and the format is load-bearing rather than
//! cosmetic. See [`TIMESTAMP_FORMAT`].

use super::super::rows::{Col, TableSpec, spec_for};
use crate::{
...
/// the precision DuckDB held; nine would invent three zeroes and imply an
...
///
/// The same eight names, in the same order, as the DuckDB file's private
/// constant. It is duplicated rather than shared only because that one is not
/// public; the ordering is asserted against the parameter count for all
/// thirteen types in the tests below, so a divergence fails there rather than
/// silently binding `updated_by` into `created_by`.
...
///
/// [`Col`]'s own accessor is private to the DuckDB module, and the variants are
/// public, so this is one match rather than a change to a file this one only
/// reads from.
...
///
/// [`Error::Invariant`] rather than [`Error::Sqlite`] because there is no
/// `rusqlite::Error` to wrap — SQLite did its job and returned the text. What
/// failed is Keel's own rule about what may be in that column, which is exactly
...
///
/// Accepts more than it writes, and the leniency has one named cause: `keel
/// migrate` copies rows out of DuckDB, whose own rendering is RFC 3339 with a
/// variable-width fraction and sometimes a space in place of the `T`. Refusing
/// those would mean the migration could not read back what it had just written.
/// Everything Keel writes itself is [`TIMESTAMP_FORMAT`].
...
///
/// A full timestamp is accepted and truncated for the same migration reason
/// [`parse_ts`] is lenient: DuckDB's `DATE` may arrive rendered as one.
...
///
/// Deliberately lenient, and the leniency is the point — carried over from the
/// DuckDB reader, where a single NULL `number` once made *every* row of that
/// type unreadable in a project, including the idempotency lookup, so
...
/// existed hold NULL, and a hand-run `keel import` can leave an empty string. A
...
mod tests {
    ...
    ///
    /// Built from the migration rather than from `SqliteStore` so these tests
    /// exercise the mapping and not the store's opening sequence — and so they
    /// keep working while the store itself is still being written.
    ...
    /// SQLite needs no `to_json` wrapper, because the column is already JSON
    /// text. Asserting the absence is worth a test: copying the DuckDB select
    /// list across would produce a statement that fails only at runtime, and
    /// only for the five types that have a list column.
    #[test]
    ...
    /// `keel migrate` copies rows that DuckDB rendered, so the reader has to
    /// accept a plain RFC 3339 string as well as the one this store writes.
    #[test]
    ...
    fn every_type_round_trips_against_the_real_schema() {
        ...
        let mut entities: Vec<Entity> = vec![
            ...
            crate::Term::new(Some(project.clone()), "keel", "This store.").into(),
