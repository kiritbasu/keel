...
//!
//! The prose half of the SQLite store, and the point of the whole move: a
//! revision, the header row it belongs to and an image attached to it are now
//! three writes in one transaction rather than writes to two engines that can
//! disagree. The cross-engine invariant `fsck` used to audit — a Lance document
//! pointing at a DuckDB row that is not there — is not repaired here, it is
//! removed. `write_revision` still checks that the entity exists, because
//! `documents.entity_id` is polymorphic and no foreign key can express "one of
//! five tables".
//!
//! # Why there is no `impl DocumentStore for SqliteStore` in this file
//!
//! The trait has seven methods and this file implements six. The seventh,
//! `search`, is the only part of the store where SQLite is not a straight
//! substitution — FTS5 and `sqlite-vec` in place of DuckDB's BM25 index and
//! Lance's vector index — so it is its own task (KEEL-126). A trait impl
//! missing a method does not compile, so these are inherent methods, named
//! exactly as the trait names them. Whoever lands `search` adds the `impl`
//! block and writes each of these six as a one-line delegation.
//!
//! **Delegate with the fully-qualified form**, `SqliteStore::revision(self, …)`
//! rather than `self.revision(…)`. Inside a trait impl the short form resolves
//! to the trait method being written, not to the inherent one, and the result
//! is a function that calls itself until the stack runs out — at runtime, with
//! no warning at the point it was written.
...
///
/// The same format [`super::rows`] uses, and for the same reason: with the
/// column TEXT, every `ORDER BY created_at` is a string comparison, and it is
/// only correct while lexicographic order matches chronological order. Chrono's
/// variable-width `%f` breaks that — `…:36.5Z` sorts *after* `…:36.524Z`,
/// because the comparison reaches `Z` against `2`.
///
/// It is written out a second time rather than shared because `rows`' own
/// helpers are private to that module and this one may not edit it while three
/// sessions have it open. The duplication is a wart with a test under it
/// ([`tests::stored_timestamps_are_fixed_width_and_sort_correctly`]); the fix is
/// to make `rows::ts` and `rows::parse_ts` `pub(super)` and delete this.
...
/// Turn stored text back into a timestamp.
///
/// Accepts more than it writes, for the one reason [`super::rows`] does:
/// `keel migrate` copies rows out of DuckDB, whose rendering is RFC 3339 with a
/// variable-width fraction and sometimes a space where the `T` should be.
fn parse_ts(table: &str, column: &str, raw: &str) -> Result<DateTime<Utc>> {
    ...
    Err(Error::Invariant {
        ...
        problem: format!("`{raw}` is not a UTC timestamp of the form 2026-08-11T09:14:36.524000Z"),
    })
}
...
///
/// Separate from [`SqliteStore::put_blob`] so that a caller holding a
...
fn insert_blob(conn: &Connection, blob: &Blob) -> Result<()> {
    ...
    .map_err(Error::sqlite(format!("store the blob {}", blob.blob_id)))?;
    Ok(())
}
...
impl SqliteStore {
    ...
    /// `current_doc_version`. Under DuckDB-and-Lance those were three writes to
    /// two engines with no way to bracket them.
    pub fn write_revision(&mut self, mut document: Document) -> Result<Document> {
        ...
        let tx = self
            .conn
            .transaction()
            .map_err(Error::sqlite("begin a revision write"))?;
        ...
        tx.commit().map_err(Error::sqlite(format!(
            "commit revision {} of {}",
            document.version, document.entity_id
        )))?;
    ...
    pub fn revision(&self, entity_id: &EntityId, version: Option<i32>) -> Result<Option<Document>> {
        ...
        let mut rows = stmt
            ...
            .map_err(Error::sqlite(format!("read a revision of {entity_id}")))?;

        match rows.next().map_err(Error::sqlite("read a document row"))? {
    ...
    pub fn revisions(&self, entity_id: &EntityId) -> Result<Vec<Document>> {
        ...
        let mut rows = stmt
            ...
            .map_err(Error::sqlite(format!("read the history of {entity_id}")))?;

        let mut out = Vec::new();
        while let Some(row) = rows.next().map_err(Error::sqlite("read a document row"))? {
    ...
    pub fn get_blob(&self, blob_id: &BlobId) -> Result<Option<Blob>> {
        let mut stmt = self
            ...
            .map_err(Error::sqlite("prepare a blob read"))?;
        let mut rows = stmt
            .query([blob_id.as_str()])
            .map_err(Error::sqlite(format!("read the blob {blob_id}")))?;

        let e = |c: &'static str| {
            let context = format!("read column `{c}` of `blobs`");
            move |source| Error::Sqlite { context, source }
        };
        match rows.next().map_err(Error::sqlite("read a blob row"))? {
            Some(row) => {
                let created_at: String = row.get("created_at").map_err(e("created_at"))?;
                Ok(Some(Blob {
...
mod tests {
    ...
    fn store_with_a_spec() -> (SqliteStore, EntityId) {
    ...
    fn current_count(store: &SqliteStore, entity_id: &EntityId) -> i64 {
        store
            .conn
            .query_row(
    ...
    /// together or not at all — which under two engines was not expressible.
    #[test]
    ...
    /// `keel migrate` copies rows DuckDB rendered, so the reader has to accept
    /// more shapes than the writer produces.
    #[test]
    ...
    /// Under two engines this was only findable by `fsck`, after the fact.
    #[test]
