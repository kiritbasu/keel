//! Backup and restore for the SQLite store: one file, and a copy of it.
//!
//! # What this replaces, and why it is so much shorter
//!
//! `backup.rs` covers two engines. It runs `EXPORT DATABASE` over DuckDB, then
//! separately dumps the Lance datasets to Parquet, and `restore` has to refuse
//! a backup missing its Lance half — because a backup that covers the rows and
//! skips the documents is not a backup.
//!
//! Those are two operations, and that is the flaw nobody could design away
//! while there were two engines: **a write landing between them produces a
//! backup that is internally inconsistent and passes every check.** The rows
//! come from one instant and the documents from another.
//!
//! `VACUUM INTO` is one operation. It takes a consistent snapshot of the whole
//! database — rows, documents, blobs, vectors and the keyword index — at a
//! single point in time, without stopping the daemon, and writes it as an
//! ordinary SQLite file. Measured at 64 ms for a 6.9 MB database.
//!
//! So the interesting property here is not that the code is shorter. It is that
//! the torn-backup failure mode stops existing.
//!
...
//! no datasets to create before they can be filled, and no type that has to be
//! cast back into shape on the way in — the `FLOAT[384]` cast that Parquet
//! forced on the Lance restore has no equivalent here, because nothing was ever
//! converted.
//!
//! # This file is called `sqlite_backup` only for now
//!
//! It sits beside `backup.rs` while both engines are in the tree. KEEL-130
//! deletes that one and renames this to `backup`.
...
pub fn backup(store: &SqliteStore, dest: impl AsRef<Path>) -> Result<BackupManifest> {
        ...
        return Err(Error::Invariant {
            ...
            problem: format!(
                "there is no `{SNAPSHOT}` in that directory. A Keel backup is one SQLite file \
                 plus a `{MANIFEST}`; if this directory holds `duckdb/` and `lance/` it is a \
                 backup from before the store moved to SQLite, and `keel migrate` reads those \
                 rather than `keel restore`"
            ),
        });
    }
...
/// nobody has.
pub fn verify_restore(store: &SqliteStore, manifest: &BackupManifest) -> Result<()> {
...
fn read_manifest(store: &SqliteStore) -> Result<BackupManifest> {
    ...
    let mut stmt = conn
        ...
        .map_err(Error::sqlite("read the applied migration list"))?;
...
mod tests {
    ...
    fn seeded(path: &Path) -> SqliteStore {
    ...
    fn a_backup_is_one_file_and_a_manifest() {
        ...
        // The whole claim of this task: one file, not a directory tree with a
        // DuckDB export beside a Lance dump.
    ...
    /// an old two-part backup is the case that will actually happen.
    #[test]
    fn a_directory_with_no_snapshot_is_refused_with_advice() {
        ...
        assert!(
            text.contains("keel migrate"),
            "an old two-part backup should be pointed at the migration path: {text}"
        );
    }
