...
//! the guarantee depends on the server rather than the kernel. `keel doctor`
...
//!
//! It is also, as the name says, *advisory*. It stops a second `keel-core` from
            ...
            Err(_) => Err(Error::Invariant {
                ...
                problem: format!(
                    "another process already has this store open for writing.\n\n\
                     Only one writer at a time: six of the seven steps in a Keel write — \
                     ...
                     It is almost always a daemon. Check with `keel doctor`, and if you meant \
