...
/// The result type every fallible `specline-core` operation returns.
pub type Result<T> = std::result::Result<T, Error>;

/// Everything that can go wrong inside `specline-core`.
...
pub enum Error {
    ...
    /// An identifier was not shaped like a Specline ULID.
    ...
    Embedding {
        ...
        /// `anyhow::Error`, which `specline-core` must not depend on.
        reason: String,
    },
}
