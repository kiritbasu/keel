...
//! make Specline unusable for someone whose setup is fine, on the strength of a
        ...
        match self {
            Hazard::SyncedFolder { service } => format!(
                "move the store off the synced tree, or exclude it in {service}'s settings. A \
                 ...
                 gets corrupted without anything failing at the time. Run `specline backup` first: \
...
/// path to exist. That keeps it testable against strings and keeps `specline-core`
...
mod tests {
    ...
    fn the_remedy_says_what_to_do() {
        ...
        assert!(remedy.contains("specline backup"), "{remedy}");
    }
}
