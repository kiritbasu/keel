...
//! caching or downloads and says nothing about Specline, embeddings or the
//! document it came from. Embedded on its own it answers questions nobody
//! asked. Prefixed with `Specline — Technical Specification › Storage ›
...
mod tests {
    ...
    fn the_embedded_text_carries_the_title_and_the_path() {
        ...
        let text = chunks[0].embed_text("Specline — Technical Specification");
        assert!(text.starts_with("Specline — Technical Specification › Storage"));
