...
//! `specline-core` validates on write; this is the audit that catches whatever
...
fn dangling_id_references(store: &Store) -> Result<Vec<String>> {
    ...
    //
    // Without this the check is worse than useless. Specline's own `B-n` decisions
...
pub fn check(store: &Store) -> Result<FsckReport> {
    ...
    if n > 0 {
        findings.push(Finding {
            ...
            remedy: "rewrite them as `blocks` with the endpoints swapped, then find \
                     whatever wrote them without going through specline-core"
                .to_owned(),
            count: n,
        });
    }
    ...
    if n > 0 {
        findings.push(Finding {
            ...
            remedy: "run `specline reembed --missing`. If it reports nothing to do, an edit \
    ...
    if n > 0 {
        findings.push(Finding {
            ...
            remedy: "DELETE FROM document_chunks WHERE doc_id NOT IN \
                     ...
                     `specline reembed --missing`. A trigger that should have deleted these \
    ...
    if n > 0 {
        findings.push(Finding {
            ...
            remedy:
                "run `specline migrate`, which rewrites them to `open`. If they came back after \
    ...
    // (D-9)". D-9 is one of *Specline's* decisions; the project it was filed under
    ...
    if n > 0 {
        findings.push(Finding {
            ...
            remedy: "nothing to repair — the history cannot be reconstructed. If the count is \
                     growing, something is writing rows outside specline-core's write path"
                .to_owned(),
            count: n,
        });
    }
...
mod citation_tests {
    ...
    fn a_citation_is_only_checked_when_the_project_titles_artifacts_that_way() {
        // The scoping that makes this check usable. Specline's own B-n decisions
