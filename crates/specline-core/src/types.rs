...
//! bug — `specline fsck` exists partly to catch that drift.
//!
...
/// at Specline's scale that is 128 bits of collision resistance against a corpus
...
/// A project key derived from a slug: `specline` → `KEEL`, `harbour` → `HARB`.
///
...
impl Project {
    ...
    /// out loud. Falls back to Specline's own word rather than to nothing — an
...
impl Task {
    ...
    /// and the ninety-four that predate the rule are reported by `specline lint`
    ...
    /// `specline claim` refuses to take.
...
pub fn validate_evidence(item: &str) -> Result<()> {
    ...
    if !EVIDENCE_KINDS.contains(&kind) {
        return Err(Error::invalid(
            EntityType::Task,
            "evidence",
            format!("`{kind}` is not a kind of evidence Specline records"),
            format!("one of {EVIDENCE_KINDS_HELP}"),
        ));
    }
