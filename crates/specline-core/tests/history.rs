...
fn a_label_is_resolved_whatever_the_type_calls_its_name_column() {
    ...
    let spec = store
        .create(
            Spec::new(project.clone(), "Specline — Technical Specification").into(),
            &prov,
        )
        .unwrap()
        .entity
    ...
    assert_eq!(out[0].label, "Specline — Technical Specification");
}
...
fn an_edge_pointing_at_nothing_keeps_the_edge_and_loses_only_the_label() {
    ...
    // Hard-delete the target behind the store's back. Nothing in Specline does
...
fn a_rows_history_is_its_own_and_reads_forwards() {
    ...
    assert_eq!(page.items[0].action, specline_core::Action::Created);
