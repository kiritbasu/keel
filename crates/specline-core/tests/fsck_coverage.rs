...
//! unreachable through `specline-core`, which is the point: the API refuses them,
...
/// about SQLite rather than about Specline, and one whose failure mode on a future
/// SQLite is a corrupted temp file rather than a useful signal. `specline doctor`
...
/// `depends_on` is never stored — `specline-core` swaps the endpoints and stores
