...
fn claiming_sets_the_status_and_records_who_and_when() {
    ...
    assert_eq!(claimed.task.status, specline_core::TaskStatus::InProgress);
...
fn closing_as_done_records_the_reason_the_message_and_the_evidence() {
    ...
    let closed = close(
        ...
        &Close {
            ...
            evidence: vec![
                ...
                "test:cargo test -p specline-core".to_owned(),
            ],
            other: None,
        },
        &session("ses_alpha"),
    ...
    assert_eq!(closed.task.status, specline_core::TaskStatus::Done);
...
fn done_needs_evidence_and_wont_do_does_not() {
    ...
    .unwrap();
    assert_eq!(f.read(&b).status, specline_core::TaskStatus::WontDo);
}
...
fn closing_as_a_duplicate_draws_the_edge_in_the_right_direction() {
    ...
    assert_eq!(closed.task.status, specline_core::TaskStatus::WontDo);
...
fn a_task_cannot_be_created_already_done_with_nothing_said() {
    ...
    born_closed.status = specline_core::TaskStatus::Done;
    ...
    assert_eq!(
        f.store
            .list(
                &specline_core::EntityQuery::in_project(f.project.clone())
                    .of_type(specline_core::EntityType::Task)
            )
            .unwrap()
            .items
            .len(),
...
fn a_task_created_done_needs_evidence_like_any_other_close() {
    ...
    no_evidence.status = specline_core::TaskStatus::Done;
...
// which is what `specline bootstrap`, `specline fixture` and adopting a finished
...
fn a_backfilled_close_is_accepted_and_keeps_the_date_it_supplied() {
    ...
    backfilled.status = specline_core::TaskStatus::Done;
...
fn a_close_on_the_way_in_is_stamped_with_a_time() {
    ...
    arriving_closed.status = specline_core::TaskStatus::WontDo;
