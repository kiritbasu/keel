...
fn fsck_notices_a_dangling_link() {
    let (store, _d, _) = loaded_store();
    // Corrupt the store behind specline-core's back, which is exactly the class of
...
// only way to reach a state specline-core now refuses to create.
...
fn a_write_at_the_seam_cannot_make_the_manifest_disagree_with_the_snapshot() {
    use specline_core::{Actor, EntityQuery, EntityStore, EntityType, Provenance, Task};
...
fn a_backup_from_an_older_schema_restores_and_migrates_forward() {
    ...
    // owner's entry point, the same way `specline backup` would after a migration.
