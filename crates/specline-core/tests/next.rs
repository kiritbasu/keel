...
impl Fixture {
    ...
    fn rank(&self) -> specline_core::NextUp {
        next::rank(&self.store, &self.project).unwrap()
    }

    fn ready(&self, filter: &specline_core::ReadyFilter) -> specline_core::Ready {
        specline_core::ready(&self.store, &self.project, filter).unwrap()
    }
    ...
    fn close(&mut self, id: &EntityId) {
        ...
        specline_core::close(
            ...
            &t.id,
            &specline_core::Close {
                reason: specline_core::CloseReason::Done,
                message: "Finished, so that what it was blocking becomes ready.".to_owned(),
                evidence: vec!["test:cargo test -p specline-core --test next".to_owned()],
                other: None,
            },
            &Provenance::anonymous(Actor::Human),
        )
...
fn a_task_with_nothing_blocking_it_is_ready() {
    ...
    // problem. Specline's own store was in exactly that state — three such tasks.
    //
...
fn ready_offers_the_children_and_not_the_parent() {
    ...
    let ready = f.ready(&specline_core::ReadyFilter::default());
...
fn unclaimed_hides_what_somebody_is_already_doing() {
    ...
    let all = f.ready(&specline_core::ReadyFilter::default());
    assert_eq!(all.items.len(), 2, "a claim does not stop work being ready");

    let unclaimed = f.ready(&specline_core::ReadyFilter {
        unclaimed: true,
        ..Default::default()
    });
...
fn labels_narrow_the_list_in_both_directions() {
    ...
    let only_desktop = f.ready(&specline_core::ReadyFilter {
    ...
    let not_desktop = f.ready(&specline_core::ReadyFilter {
    ...
    let both_wanted = f.ready(&specline_core::ReadyFilter {
...
fn a_limited_list_reports_that_it_was_cut_and_how_much_there_was() {
    ...
    let cut = f.ready(&specline_core::ReadyFilter {
        limit: Some(2),
        ..Default::default()
    });
    ...
    let whole = f.ready(&specline_core::ReadyFilter {
        limit: Some(5),
        ..Default::default()
    });
    assert!(
...
/// A blocker Specline cannot read must not be treated as a blocker that is not
/// there.
///
