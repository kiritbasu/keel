...
fn one_of_each(project_id: &EntityId, metric_id: &EntityId) -> Vec<Entity> {
    ...
    task.external_refs = vec!["https://github.com/kb/specline/pull/1".into()];
    ...
    decision.decided_at = Some(specline_core::now());
    ...
    feedback.occurred_at = Some(specline_core::now());
    ...
    environment.last_deployed_at = Some(specline_core::now());
...
fn a_fresh_store_opens_and_migrates() {
    ...
    assert_eq!(
        applied,
        specline_core::store::schema::migrations().len() as i64,
...
fn every_entity_type_round_trips() {
    ...
    for entity in one_of_each(&project_id, &metric_id) {
        ...
        let field = match entity_type {
            ...
            EntityType::Project => ("name", json!("Specline")),
        };
...
///
/// The rule lives in `specline-core` rather than in the MCP layer so that the CLI,
/// the daemon and `specline import` cannot disagree about what is storable — two
