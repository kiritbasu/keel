...
fn the_mcp_server_talks_to_the_daemon_over_http() {
    ...
    let keel = &mcp["mcpServers"]["keel"];
    assert_eq!(
        keel["type"], "http",
        "a stdio server would be a second writer"
    );
    let url = keel["url"].as_str().expect("the server has a url");
...
fn the_shipped_scripts_parse() {
    for script in ["plugin/scripts/setup.sh", "plugin/hooks/keel-hook.sh"] {
        let shell = if script.ends_with("keel-hook.sh") {
