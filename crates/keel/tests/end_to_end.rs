...
/// A `keel-daemon` process and the address it actually bound.
struct Daemon {
    child: Child,
    base: String,
...
impl Daemon {
    ...
    fn start() -> Daemon {
        ...
        let mut child = Command::new(env!("CARGO_BIN_EXE_keel-daemon"))
            .arg("--home")
            .arg(home.path())
            .args(["--bind", "127.0.0.1:0"])
            .env_remove("KEEL_HOME")
            .env_remove("KEEL_BIND")
            .env("RUST_LOG", "keel_daemon=info")
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .expect("spawn keel-daemon");
...
fn the_event_stream_opens_and_says_something() {
    ...
    assert!(
        String::from_utf8_lossy(&buffer[..read]).contains("keel"),
