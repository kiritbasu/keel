...
//!
//! `keel hook session-start` and `keel hook stop`. They were 317 lines of bash
...
//! path exited 0 silently, and the result was indistinguishable from Keel not
...
/// model-invoked, and across thirty headless sessions with `keel` installed and
...
const PREAMBLE: &str = "Keel holds this project's specs, decisions, tasks, questions and history. \
You did not have to ask for this — it is here so you start oriented. Write back to it when \
something becomes true, with the keel_* tools; the `keel` skill has the detail on what belongs \
where.\n\n\
...
fn session_hint(claude_session: &str) -> String {
    format!(
        "Use exactly this on every Keel call: session_id = \"ses_{claude_session}\". It is this \
...
/// is that Keel was out of mind entirely. It used to spend a paragraph
/// re-teaching what to write, which the session-start hook and the skill had
/// both already covered.
const NUDGE: &str = "Nothing from this session reached Keel. If anything became true here, \
...
pub fn stop(daemon: &str) {
    ...
    // Is this directory a Keel project at all? KEEL-192: the activity check
...
mod tests {
    ...
    fn an_empty_summary_injects_nothing() {
        assert!(
            session_start_context(&digest("   ", json!({"slug": "keel"})), Some("a")).is_none()
        );
    ...
    fn the_session_id_is_pinned_to_claude_codes_own() {
        let body = digest("something", json!({"slug": "keel"}));
