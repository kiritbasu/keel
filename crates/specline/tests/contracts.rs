...
//! byte-identically across a hundred runs. The tenth, `specline generate`, differs on
...
//!
//! Stripping the banner is not a normalisation invented here. `specline generate
...
use specline_core::{EntityQuery, EntityStore, EntityType, Store, fixture, generate};
...
fn tools_description() -> String {
    let tools: Vec<serde_json::Value> = specline_mcp::all_tools()
        .iter()
        .map(|t| {
            serde_json::json!({
                "name": t.name,
...
/// process. `specline` is a binary crate with no library half, so its `Cli` type
...
fn cli_description() -> String {
    let exe = env!("CARGO_BIN_EXE_specline");
...
fn help_for(exe: &str, args: &[&str]) -> String {
    let output = std::process::Command::new(exe)
        ...
        // `[env: SPECLINE_HOME=<the value right now>]` — the *current* value, not
        ...
        // `SPECLINE_HOME`, `SPECLINE_BIND` or `SPECLINE_DAEMON_URL` exported — which is a
        ...
        // found when CI set `SPECLINE_HOME` to a scratch directory to keep the test
        ...
        .output()
        .expect("run the built specline binary");
    assert!(
        output.status.success(),
        "`specline {} --help` failed: {}",
        args.join(" "),
...
fn every_surface_matches_its_recorded_description() {
    ...
    assert!(
        ...
        "the contract descriptions are out of date:\n  {}\n\n\
         ...
         \x20   UPDATE_CONTRACTS=1 cargo test -p specline --test contracts\n\n\
...
///
/// `manifest.json` is excluded and the `specline:generated` banner line is dropped
    ...
    while let Some(dir) = stack.pop() {
        ...
        for path in entries {
            ...
            let stripped: String = body
                .lines()
                .filter(|l| !l.contains("specline:generated"))
                .map(redact_ids)
...
/// Replace every Specline id with a placeholder.
///
...
/// measurement ran `specline generate` against a store built once, so it never saw
/// it.
///
...
///
/// Anchored to Specline's own shape — three lowercase letters, an underscore, then
