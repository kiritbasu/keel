...
//! take the store lock, which meant a `specline generate` holding that lock for
...
async fn the_schema_is_reported_without_the_store() {
    ...
    let busy = tokio::time::timeout(Duration::from_secs(2), health(&base))
        .await
        .expect("health must not wait for the store lock");
    assert_eq!(busy["schema"], specline_core::shipped_schema_version());
...
async fn applying_an_update_that_is_not_staged_is_refused() {
    ...
    let token = specline_core::token::read(home.path())
...
async fn health_says_whether_update_checking_is_happening_at_all() {
    ...
    //
    // Asserted as "the absolute path of the running executable", not as "a path
    // with the product's name in it". The name spelling passed here for the
    // wrong reason until the rename: under the test harness the executable is
    // the *test* binary, and the substring matched only because this checkout
    // happens to sit in a directory named after the product. On a machine that
    // cloned it anywhere else the check was vacuous, and the rename is what
    // finally made it say so.
    let reported = body["executable"]
        .as_str()
        .unwrap_or_else(|| panic!("health must name the binary it is running: {body}"));
    assert_eq!(
        std::path::Path::new(reported),
        std::env::current_exe().expect("the running executable has a path"),
        "health should report the executable it is actually running: {body}"
    );
    drop(state);
}

/// KEEL-220. Two of the three release targets cannot link the ONNX runtime, so
/// `specline 0.1.x` on Intel macOS and `specline 0.1.x` on arm64 are different binaries
...
async fn health_says_whether_this_build_can_embed_at_all() {
    ...
    assert_eq!(
        embeddings["built_in"],
        serde_json::json!(specline_daemon::EMBEDDINGS_BUILT_IN),
...
async fn a_build_without_a_model_never_reports_one_as_loaded() {
    ...
    if !specline_daemon::EMBEDDINGS_BUILT_IN {
        assert_ne!(
