...
/// (`#/projects/specline`), so the server sees `/` for every screen and the
...
fn respond(path: &str, file: rust_embed::EmbeddedFile, token: &str) -> Response {
    ...
    let body = if path == "index.html" {
        ...
        let meta = format!(
            "<meta name=\"specline-token\" content=\"{}\">",
...
mod tests {
    ...
    async fn get(path: &str) -> Response {
        ...
        let store = specline_core::Store::in_memory().expect("an in-memory store");
    ...
    async fn an_unknown_route_serves_the_app_shell() {
        let response = get("/projects/specline/board").await;
        assert_eq!(response.status(), StatusCode::OK);
        assert!(header_of(&response, "content-type").contains("text/html"));
    }
