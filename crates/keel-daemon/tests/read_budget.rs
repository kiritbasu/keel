...
use keel_core::{Actor, EntityQuery, EntityStore, EntityType, NewNote};
...
impl Daemon {
    ...
    async fn start() -> Self {
        ...
        {
            let mut store =
                keel_core::Store::open(dir.path().join("keel.sqlite")).expect("open the store");
            ...
            let tasks = store
                ...
                .expect("list tasks");
            let provenance = keel_core::Provenance {
                actor: Actor::Claude,
                session_id: Some("ses_budget".to_owned()),
                surface: Some(keel_core::Surface::Code),
            };
...
fn project_id(store: &keel_core::Store, slug: &str) -> keel_core::EntityId {
    let page = store
        .list(
            &EntityQuery::default()
    ...
    page.items
        .iter()
        .find_map(|e| match e {
            keel_core::Entity::Project(p) if p.slug == slug => Some(p.id.clone()),
            _ => None,
        })
