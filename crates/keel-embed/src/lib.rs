...
//!
//! Split out of `keel-core` because that crate's contract says it never opens a
...
//!
//! The split is not tidiness. Everything linking `keel-core` used to pull in
//! ONNX Runtime whether or not it ever embedded anything — the CLI, the MCP
//! crate, every test binary. Only the daemon and `keel reembed` construct one.
//!
//! [`keel_core::Embedder`] and the test `HashEmbedder` stay in core, because
...
impl FastEmbedder {
    ...
    pub fn new(cache_dir: impl Into<std::path::PathBuf>) -> Result<Self> {
        ...
        Ok(FastEmbedder {
            ...
            model_name: keel_core::EMBEDDING_MODEL.to_owned(),
            dimensions: keel_core::EMBEDDING_DIM,
        })
    }
}

impl keel_core::Embedder for FastEmbedder {
